...
type InsightsFlagEventsBetaApi interface {
...
For example, use `?expand=experiments` to include the `experiments` field in the response. By default, this field is **not** included in the response.
	...
	*/
	GetFlagEvents(ctx context.Context) ApiGetFlagEventsRequest

	// GetFlagEventsExecute executes the request
	//  @return FlagEventCollectionRep
	GetFlagEventsExecute(r ApiGetFlagEventsRequest) (*FlagEventCollectionRep, *http.Response, error)
}
...
type ApiGetFlagEventsRequest struct {
	ctx context.Context
	ApiService InsightsFlagEventsBetaApi
	projectKey *string
