...
type ContextSettingsApi interface {
...
If you previously patched the flag, and the patch included the context's data, LaunchDarkly continues to use that data. If LaunchDarkly has never encountered the combination of the context's key and kind before, it calculates the flag values based on the context kind and key.
	...
	*/
	PutContextFlagSetting(ctx context.Context, projectKey string, environmentKey string, contextKind string, contextKey string, featureFlagKey string) ApiPutContextFlagSettingRequest

	// PutContextFlagSettingExecute executes the request
	PutContextFlagSettingExecute(r ApiPutContextFlagSettingRequest) (*http.Response, error)
}
...
func (r ApiPutContextFlagSettingRequest) Execute() (*http.Response, error) {
	return r.ApiService.PutContextFlagSettingExecute(r)
}
...
func (a *ContextSettingsApiService) PutContextFlagSettingExecute(r ApiPutContextFlagSettingRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/projects/{projectKey}/environments/{environmentKey}/contexts/{contextKind}/{contextKey}/flags/{featureFlagKey}"
	...
	localVarPath = strings.Replace(localVarPath, "{"+"contextKind"+"}", url.PathEscape(parameterToString(r.contextKind, "")), -1)
