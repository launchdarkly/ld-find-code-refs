...
type TagsApi interface {
	...
	*/
	...
	GetTagsExecute(r ApiGetTagsRequest) (*TagsCollection, *http.Response, error)
}
...
func (r ApiGetTagsRequest) AsOf(asOf string) ApiGetTagsRequest {
	r.asOf = &asOf
	return r
}

func (r ApiGetTagsRequest) Execute() (*TagsCollection, *http.Response, error) {
...
//  @return TagsCollection
func (a *TagsApiService) GetTagsExecute(r ApiGetTagsRequest) (*TagsCollection, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *TagsCollection
	)
	...
	if r.kind != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("kind", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("kind", parameterToString(t, "multi"))
		}
	}
	if r.pre != nil {
		localVarQueryParams.Add("pre", parameterToString(*r.pre, ""))
	}
	if r.archived != nil {
	...
	if r.asOf != nil {
		localVarQueryParams.Add("asOf", parameterToString(*r.asOf, ""))
	}
