...
func (a *CustomRolesApiService) DeleteCustomRoleExecute(r ApiDeleteCustomRoleRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/roles/{customRoleKey}"
	localVarPath = strings.Replace(localVarPath, "{"+"customRoleKey"+"}", url.PathEscape(parameterToString(r.customRoleKey, "")), -1)
...
func (r ApiPostCustomRoleRequest) CustomRolePost(customRolePost CustomRolePost) ApiPostCustomRoleRequest {
	r.customRolePost = &customRolePost
	return r
}
...
func (a *CustomRolesApiService) PostCustomRoleExecute(r ApiPostCustomRoleRequest) (*CustomRole, *http.Response, error) {
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.customRolePost == nil {
		return localVarReturnValue, nil, reportError("customRolePost is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.customRolePost
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKey"]; ok {
