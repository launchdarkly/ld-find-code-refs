...
All dotfiles and patterns in `.gitignore` and `.ignore` will be excluded by default, except the `.github` directory. Flags may be referenced when using [launchdarky/gha-flags](https://github.com/launchdarkly/gha-flags). If you would like to skip scanning these file, add `.github` to one of the ignore files.
