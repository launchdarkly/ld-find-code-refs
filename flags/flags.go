...
func GetFlagKeys(opts options.Options, repoParams ld.RepoParams) map[string][]string {
	...
	for _, proj := range opts.Projects {
		flags, err := getFlags(ldApi, proj.Key, opts.FlagState)
		if err != nil {
...
func getFlags(ldApi ld.ApiClient, projKey, flagState string) ([]string, error) {
	flags, err := ldApi.GetFlagKeyList(projKey, flagState)
	if err != nil {
		return nil, err
	}
	return flags, nil
