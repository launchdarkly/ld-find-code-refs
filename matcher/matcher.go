...
func Scan(opts options.Options) {
	...
	flagMatcher := coderefs.GenerateSearchElements(opts, repoParams)
	...
	refs, err := search.SearchForRefs(opts.ProjKey, matcher)
	if err != nil {
	...
	outDir := opts.OutDir
	if outDir != "" {
		outPath, err := branch.WriteToCSV(outDir, opts.ProjKey, repoParams.Name, revision)
		if err != nil {
	...
	case err != nil:
		helpers.FatalServiceError(fmt.Errorf("error sending code references to LaunchDarkly: %w", err), ignoreServiceErrors)
	}
	...
	if gitClient != nil {
		...
		if err != nil {
			...
			if err != nil {
				helpers.FatalServiceError(fmt.Errorf("failed to mark old branches for code reference pruning: %w", err), ignoreServiceErrors)
			}
		}
	}
}
