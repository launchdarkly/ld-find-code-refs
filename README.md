...
Branch cleanup requires git ssh credentials to have already been setup on the machine executing `ld-find-code-refs`. Branch cleanup is not currently supported when running `ld-find-code-refs` via Github actions or Bitbucket pipelines.
